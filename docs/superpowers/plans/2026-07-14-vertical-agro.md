...
- **OBRIGATÓRIO — header `Host` em toda request de usuário não-super_admin:** o middleware `requireAuth` resolve o tenant pelo slug do Host (`host.split('.')[0]`) e devolve **403** se o Host não bater com a org do usuário. Todo teste com usuário `admin`/`secretaria` DEVE: (1) capturar o slug ao criar a org, (2) montar `const host = \`${slug}.orrin.com.br\``, (3) chamar `.set('Host', host)` em **cada** request, junto do `.set('Authorization', ...)`. Sem isso todo request 403 (é o motivo de `servicos.test.ts` — teste legado sem Host — falhar). Referência viva: `backend/tests/agendamentos-timezone.test.ts`. Nos templates deste plano que fazem só `.set('Authorization', ...)`, adicionar o `.set('Host', host)` correspondente. Testes que exercitam funções diretamente (sem supertest, ex.: Task 10 `executarToolAgro`) não precisam de Host.
- Critério de não-regressão realista: a suíte NÃO fica 100% verde nesta base. **Baseline pré-existente medido na `main`: 12 arquivos / 23 testes falhando** (`admin`, `agendamento-detalhe`, `agendamentos`, `atendimentos-chat`, `atendimentos`, `claude-agent`, `confirmacao`, `dashboard`, `pacientes`, `profissionais`, `servicos` com 403 de Host/seed legado; `webhook` com 2 timeouts). São testes pré-multi-tenant que dependem de seed/conta compartilhada ausente — **não são bugs e não devem ser "consertados"**. O gate é **"nenhuma falha NOVA além dessas 23"**.
- **Testes novos vão em ARQUIVOS NOVOS, nunca anexados aos legados.** Os arquivos legados (`pacientes.test.ts`, `servicos.test.ts`, `admin.test.ts` fora do path super_admin, etc.) têm fixtures quebrados (usuário sem `tenant_id`, sem Host). Todo `describe` novo do agro cria sua própria org + usuário com `tenant_id` + Host próprio. Correções de destino de arquivo: **Task 5 → `pacientes-agro.test.ts` (novo)**, **Task 13 (teste backend) → mesmo `pacientes-agro.test.ts`**, em vez de anexar a `pacientes.test.ts`. Cada subagente roda SÓ o seu arquivo novo (`npx vitest run tests/<novo>.test.ts`), não a suíte inteira.
