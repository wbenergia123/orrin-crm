...
- Critério de não-regressão realista: a suíte não fica 100% verde nesta base (`servicos.test.ts` já falha por ser legado sem Host). O gate é **"nenhuma falha nova em relação à `main`"**, não "zero falhas". Antes de começar, rodar `npm test` na base e anotar o baseline de falhas pré-existentes.
