// mock-server.cjs — servidor mock temporário para preview visual
...
// ── Profissionais ──
const profissionais = [
  { id: 'pr1', nome: 'Dr. Carlos Oliveira', especialidade: 'Dermatologia', ativo: true },
  { id: 'pr2', nome: 'Dra. Ana Souza', especialidade: 'Estética', ativo: true },
]
app.get('/api/profissionais', (_req, res) => res.json(profissionais))
app.post('/api/profissionais', (req, res) => {
  const novo = { id: `pr${Date.now()}`, ...req.body, ativo: true }
  profissionais.push(novo)
  res.status(201).json(novo)
})
app.patch('/api/profissionais/:id', (req, res) => {
  const idx = profissionais.findIndex(p => p.id === req.params.id)
  if (idx >= 0) profissionais[idx] = { ...profissionais[idx], ...req.body }
  res.json(profissionais[idx] || {})
})

// ── Serviços ──
const servicos = [
  { id: 's1', nome: 'Avaliação Inicial', preco: 200, duracao_minutos: 30, ativo: true },
  { id: 's2', nome: 'Botox', preco: 800, duracao_minutos: 45, ativo: true },
  { id: 's3', nome: 'Preenchimento', preco: 1200, duracao_minutos: 60, ativo: true },
  { id: 's4', nome: 'Limpeza de Pele', preco: 350, duracao_minutos: 60, ativo: true },
  { id: 's5', nome: 'Bioestimulador', preco: 1500, duracao_minutos: 60, ativo: true },
]
app.get('/api/servicos', (_req, res) => res.json(servicos))
app.post('/api/servicos', (req, res) => {
  const novo = { id: `s${Date.now()}`, ...req.body, ativo: true }
  servicos.push(novo)
  res.status(201).json(novo)
})
app.patch('/api/servicos/:id', (req, res) => {
  const idx = servicos.findIndex(s => s.id === req.params.id)
  if (idx >= 0) servicos[idx] = { ...servicos[idx], ...req.body }
  res.json(servicos[idx] || {})
})

// ── Agenda (agendamentos completos) ──
const agenda = [
  { id: 'ag1', paciente_id: 'p1', servico_id: 's2', profissional_id: 'pr1', data_hora: new Date(Date.now() + 2*3600000).toISOString(), status: 'confirmado', notas: 'Retorno botox', paciente: { id: 'p1', nome: 'Maria Silva', telefone: '(11) 99999-1111' }, servico: { id: 's2', nome: 'Botox', preco: 800 }, profissional: { id: 'pr1', nome: 'Dr. Carlos Oliveira' } },
  { id: 'ag2', paciente_id: 'p2', servico_id: 's1', profissional_id: 'pr2', data_hora: new Date(Date.now() + 5*3600000).toISOString(), status: 'agendado', notas: null, paciente: { id: 'p2', nome: 'João Santos', telefone: '(11) 99999-2222' }, servico: { id: 's1', nome: 'Avaliação Inicial', preco: 200 }, profissional: { id: 'pr2', nome: 'Dra. Ana Souza' } },
  { id: 'ag3', paciente_id: 'p3', servico_id: 's3', profissional_id: 'pr1', data_hora: new Date(Date.now() + 24*3600000).toISOString(), status: 'agendado', notas: null, paciente: { id: 'p3', nome: 'Ana Costa', telefone: '(11) 99999-3333' }, servico: { id: 's3', nome: 'Preenchimento', preco: 1200 }, profissional: { id: 'pr1', nome: 'Dr. Carlos Oliveira' } },
  { id: 'ag4', paciente_id: 'p1', servico_id: 's4', profissional_id: 'pr2', data_hora: new Date(Date.now() - 24*3600000).toISOString(), status: 'concluido', notas: 'Paciente satisfeita', paciente: { id: 'p1', nome: 'Maria Silva', telefone: '(11) 99999-1111' }, servico: { id: 's4', nome: 'Limpeza de Pele', preco: 350 }, profissional: { id: 'pr2', nome: 'Dra. Ana Souza' } },
]

app.get('/api/agenda', (req, res) => {
  let result = [...agenda]
  if (req.query.paciente_id) result = result.filter(a => a.paciente_id === req.query.paciente_id)
  if (req.query.profissional_id) result = result.filter(a => a.profissional_id === req.query.profissional_id)
  if (req.query.status) result = result.filter(a => a.status === req.query.status)
  res.json(result)
})
app.post('/api/agenda', (req, res) => {
  const paciente = pacientes.find(p => p.id === req.body.paciente_id)
  const servico = servicos.find(s => s.id === req.body.servico_id)
  const profissional = profissionais.find(p => p.id === req.body.profissional_id)
  const novo = { id: `ag${Date.now()}`, ...req.body, status: 'agendado', paciente, servico, profissional }
  agenda.push(novo)
  res.status(201).json(novo)
})
app.patch('/api/agenda/:id', (req, res) => {
  const idx = agenda.findIndex(a => a.id === req.params.id)
  if (idx >= 0) agenda[idx] = { ...agenda[idx], ...req.body }
  res.json(agenda[idx] || {})
})
...
// ── Atendimentos (histórico de conversas/atendimentos) ──
const atendimentosChat = [
  { id: 'atd1', paciente_id: 'p1', paciente: { nome: 'Maria Silva', telefone: '(11) 99999-1111' }, ultima_mensagem: 'Botox agendado com sucesso!', modo_humano: false, created_at: new Date(Date.now() - 3600000).toISOString() },
  { id: 'atd2', paciente_id: 'p2', paciente: { nome: 'João Santos', telefone: '(11) 99999-2222' }, ultima_mensagem: 'Quando posso vir?', modo_humano: false, created_at: new Date(Date.now() - 7200000).toISOString() },
]
app.get('/api/atendimentos', (req, res) => {
  if (req.query.paciente_id) {
    const filtered = atendimentosChat.filter(a => a.paciente_id === req.query.paciente_id)
    return res.json(filtered)
  }
  res.json(atendimentosChat)
})

// ── Configurações ──
let config = { empresa_nome: 'Clínica Orrin', email_contato: 'contato@orrin.com.br', telefone: '(11) 3000-0000', prompt_pedro: 'Você é a Ana, assistente da clínica. Seja educada e ajude os pacientes.', timezone: 'America/Sao_Paulo', whatsapp_conectado: false }
app.get('/api/configuracoes', (_req, res) => res.json(config))
app.patch('/api/configuracoes', (req, res) => { config = { ...config, ...req.body }; res.json(config) })
